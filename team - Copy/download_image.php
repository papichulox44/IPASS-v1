<?php
	...
	if(ISSET($_REQUEST['req_comment_id']))
	{
		$file = $_REQUEST['req_comment_id'];

		$results = mysqli_query($conn, "SELECT * FROM requirement_comment WHERE `comment_id`='$file'");
		$rows = mysqli_fetch_array($results);
	
		header("Content-Disposition: attachment; filename=".$rows['comment_attactment']);
		header("Content-Type: application/octet-stream;");
		readfile("../assets/media/requirements/".$rows['comment_attactment']);
	}
?>