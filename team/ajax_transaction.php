<?php
            ...
            if (isset($filterby) or isset($get_from))
                {
                    ...
                    else if($filterby == "This Week")
                    {
                        ...
                        $results = mysqli_query($conn, "SELECT task.task_name, space.space_name, list.list_name, finance_phase.phase_name, finance_transaction.val_date, finance_transaction.val_method, finance_transaction.val_transaction_no, finance_transaction.val_currency, finance_transaction.val_charge, finance_transaction.val_amount, finance_transaction.val_initial_amount, finance_transaction.val_usd_rate, finance_transaction.val_usd_total, finance_transaction.val_php_rate, finance_transaction.val_php_total, finance_transaction.val_client_rate, finance_transaction.val_client_total, finance_remarks.remarks_value,finance_transaction.val_assign_to, finance_transaction.val_phase_id, finance_transaction.val_id, finance_transaction.val_remarks FROM finance_phase INNER JOIN finance_transaction ON finance_transaction.val_phase_id = finance_phase.phase_id INNER JOIN task ON finance_transaction.val_assign_to = task.task_id LEFT JOIN finance_remarks ON finance_remarks.remarks_phase_id = finance_transaction.val_phase_id AND finance_remarks.remarks_to = finance_transaction.val_assign_to INNER JOIN list ON task.task_list_id = list.list_id INNER JOIN space ON list.list_space_id = space.space_id $remarks AND finance_transaction.val_date BETWEEN '$from' AND '$to' ORDER BY task.task_name ASC, finance_phase.phase_name ASC, list.list_name ASC, finance_transaction.val_date DESC");
                    ...
                    else if($filterby == "Custom Date")
                    {
                        ...
                        $results = mysqli_query($conn, "SELECT task.task_name, space.space_name, list.list_name, finance_phase.phase_name, finance_transaction.val_date, finance_transaction.val_method, finance_transaction.val_transaction_no, finance_transaction.val_currency, finance_transaction.val_charge, finance_transaction.val_amount, finance_transaction.val_initial_amount, finance_transaction.val_usd_rate, finance_transaction.val_usd_total, finance_transaction.val_php_rate, finance_transaction.val_php_total, finance_transaction.val_client_rate, finance_transaction.val_client_total, finance_remarks.remarks_value,finance_transaction.val_assign_to, finance_transaction.val_phase_id, finance_transaction.val_id, finance_transaction.val_remarks FROM finance_phase INNER JOIN finance_transaction ON finance_transaction.val_phase_id = finance_phase.phase_id INNER JOIN task ON finance_transaction.val_assign_to = task.task_id LEFT JOIN finance_remarks ON finance_remarks.remarks_phase_id = finance_transaction.val_phase_id AND finance_remarks.remarks_to = finance_transaction.val_assign_to INNER JOIN list ON task.task_list_id = list.list_id INNER JOIN space ON list.list_space_id = space.space_id $remarks AND finance_transaction.val_date BETWEEN '$get_from' AND '$get_to' ORDER BY task.task_name ASC, finance_phase.phase_name ASC, list.list_name ASC, finance_transaction.val_date DESC");
                    }
                }
                ...
                {
                  $val_phase_id = $rows['val_phase_id'];
                  $user_id = $_SESSION['user'];
                  $query_filter_status = mysqli_query($conn, "SELECT * FROM filter_status WHERE user_id = $user_id AND filter_name = 'transaction'");
                  ...
                  if (mysqli_num_rows($query_filter_status) === 1) {
                    ...
                    if(count($value_array) === 1)
                    {
                      ...
                      if ($value1 === $val_phase_id) {
                        include 'ajax_transaction_table.php';
                      }
                    }
                    else if(count($value_array) === 2)
                    {
                      ...
                      if ($value1 === $val_phase_id OR $value2 === $val_phase_id) {
                        include 'ajax_transaction_table.php';
                      }
                    }
                    else if(count($value_array) === 3)
                    {
                      ...
                      if ($value1 === $val_phase_id OR $value2 === $val_phase_id OR $value3 === $val_phase_id) {
                        include 'ajax_transaction_table.php';
                      }
                    }
                    else if(count($value_array) === 4)
                    {
                      ...
                      if ($value1 === $val_phase_id OR $value2 === $val_phase_id OR $value3 === $val_phase_id OR $value4 === $val_phase_id) {
                        include 'ajax_transaction_table.php';
                      }
                    }
                    else if(count($value_array) === 5)
                    {
                      ...
                      if ($value1 === $val_phase_id OR $value2 === $val_phase_id OR $value3 === $val_phase_id OR $value4 === $val_phase_id OR $value5 === $val_phase_id) {
                        include 'ajax_transaction_table.php';
                      }
                    }
                    else if(count($value_array) === 6)
                    {
                      ...
                      if ($value1 === $val_phase_id OR $value2 === $val_phase_id OR $value3 === $val_phase_id OR $value4 === $val_phase_id OR $value5 === $val_phase_id OR $value5 === $val_phase_id) {
                        include 'ajax_transaction_table.php';
                      }
                    }
                  } else {
    ...
    if(isset($_POST['fetch_per_transaction']))
    {
        ...
        $transaction_no = $fetch_col_name['val_transaction_no'];
        $phase_id = $fetch_col_name['val_phase_id'];
        $task_id = $fetch_col_name['val_assign_to'];
        ?>
                ...
                <div class="col-md-6">
                    <div class="form-group row">
                        ...
                        <div class="col-md-1">
                            ...
                            <input type="hidden" class="form-control text-center" id="disc_id" readonly value="<?php echo $fetch_col_name['discount_id']; ?>">
                        </div>
                        <div class="col-md-4">
                        ...
                        <div class="col-md-3">
                            <label for="contact1-lastname" class="text-center">Action</label><br>
                            <button class="btn btn-sm btn-noborder btn-danger text-center" id="<?php echo $val_id; ?>,<?php echo $phase_id; ?>,<?php echo $transaction_no; ?>,<?php echo $task_id; ?>" onclick="delete_transaction(this.id)"><i class="fa fa-trash"></i></button>
                        </div>
                    </div>
    ...
    if (isset($_POST['delete_transaction'])) {
        ...
        $result = mysqli_query($conn, "SELECT * FROM finance_phase WHERE phase_id = $phase_id") or die(mysqli_error());
        $data = mysqli_fetch_assoc($result);
        $phase_name = $data['phase_name'];
        $comment = 'Transaction Deleted "'.$phase_name.'" | Transaction Number: '.$transaction_no.'';

        $delete_transaction = mysqli_query($conn, "DELETE from finance_transaction WHERE val_id = '$val_id'") or die(mysqli_error());
        if ($delete_transaction) {
            $insert_comment = mysqli_query($conn, "INSERT INTO comment (comment_task_id, comment_user_id, comment_message, comment_date, comment_type) VALUES ($task_id, $user_id, '$comment', NOW(), 1)") or die(mysqli_error());
            if ($insert_comment) {
                echo 'success';
            }
        }
    ...
    if(isset($_POST['filter_services']))
    {
        ...
        $result = mysqli_query($conn, "SELECT * FROM finance_phase WHERE phase_space_id = '$space_id'") or die(mysqli_error());
        while($rows = mysqli_fetch_array($result))
        {
          echo '
          <button class="dropdown-item">
          <input type="checkbox" id="status" value="'.$rows['phase_id'].'">
          <label>'.$rows['phase_name'].'</label>
          </button>
          ';
        }
        echo '
          <button class="btn btn-success form-control" onclick="filter_phase()">
          Filter
          </button>
        ';
    }
    ...
    if(isset($_POST['filter_phase']))
    {
        ...
        $delete_filter = mysqli_query($conn, "DELETE FROM filter_status WHERE user_id = $user_id AND filter_name = 'transaction'") or die(mysqli_error());
        $result = mysqli_query($conn, "INSERT INTO filter_status (user_id, array_status, filter_name) VALUES ($user_id, '$array_status_id', 'transaction')") or die(mysqli_error());
        if ($result) {
          echo 'success';
        }
    }
